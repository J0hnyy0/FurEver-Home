...
function togglePasswordVisibility(inputId) {
  ...
  const btn        = input.closest('.password-input-wrapper')?.querySelector('.toggle-password-btn');
...
// ⚠️ Module scripts are scoped — togglePasswordVisibility and closeErrorBanner
// are wired up via data attributes in DOMContentLoaded below (no inline onclick needed)
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.querySelectorAll('[data-close-banner]').forEach(btn => {
    btn.addEventListener('click', () => closeErrorBanner(btn.dataset.closeBanner));
  });
