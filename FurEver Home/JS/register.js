...
window.togglePasswordVisibility = function(inputId) {
  ...
  if (!wrapper) {
    console.warn(`Wrapper not found for ${inputId}`);
    return;
  }

  const toggleBtn = wrapper.querySelector('.toggle-password-btn');
  if (!toggleBtn) {
    console.warn(`Toggle button not found for ${inputId}`);
    return;
  }
...
async function handleSignUp(e) {
  ...
  // Validation
  if (!fullName)             return showErrorBanner('signup', 'Please enter your full name.');
  if (!isValidEmail(email))  return showErrorBanner('signup', 'Please enter a valid email.');
  if (!username)             return showErrorBanner('signup', 'Please enter a username.');
  if (!phoneNumber)          return showErrorBanner('signup', 'Please enter phone number.');
  if (phoneNumber.length < 10) return showErrorBanner('signup', 'Phone number must be at least 10 digits.');
...
document.addEventListener('click', (e) => {
  const closeBtn = e.target.closest('[data-close-banner]');
  if (closeBtn) {
    const bannerType = closeBtn.getAttribute('data-close-banner');
    closeErrorBanner(bannerType);
  }
});
