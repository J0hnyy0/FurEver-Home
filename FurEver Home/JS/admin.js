...
async function loadDashboardData() {
  ...
  try {
    ...
    const applicationsCount = await getCount('adoptionApplications');
...
async function loadApplications() {
  ...
  try {
    ...
    const snap = await getDocs(collection(db, 'adoptionApplications'));
    ...
    snap.forEach(docSnap => {
      ...
      const actions = status === 'pending' 
        ? `
          <button class="btn-approve" onclick="approveApplication('${app.id}', '${(app.fullName || 'this user').replace(/'/g, "\\'")}')">Approve</button>
          <button class="btn-reject" onclick="rejectApplication('${app.id}', '${(app.fullName || 'this user').replace(/'/g, "\\'")}')">Reject</button>
        ` : '';
...
window.viewApplicationDetails = async function(appId) {
  try {
    const ref = doc(db, 'adoptionApplications', appId);
    const snap = await getDoc(ref);
    ...
    document.getElementById('applicationDetailContent').innerHTML = `
      ...
      ${app.status === 'pending' ? `
        <div class="modal-footer" style="margin-top: 20px;">
          <button class="btn-approve" onclick="approveApplication('${appId}', '${(app.fullName || 'this user').replace(/'/g, "\\'")}'); closeApplicationDetailModal();">Approve</button>
          <button class="btn-reject" onclick="rejectApplication('${appId}', '${(app.fullName || 'this user').replace(/'/g, "\\'")}'); closeApplicationDetailModal();">Reject</button>
        </div>
      ` : ''}
    `;
...
window.approveApplication = async function(id, applicantName) {
  showConfirmationModal(`Approve adoption application from ${applicantName}?`, async () => {
    try {
      await updateDoc(doc(db, 'adoptionApplications', id), { 
        status: 'approved'
      });
